...
//////////////////////////////////////////////////////////////////////////////////	 
//SIZE CharSize[3]={
//	                {8,16,16,16,32}, //字符8*16 汉字16*16
//	                {16,24,24,48,72} //字符16*24 汉字24*24
//	               };

//LCD的画笔颜色和背景色	   
u16 POINT_COLOR =0x0000;	//画笔颜色
u16 BACK_COLOR = CYAN;  //背景色 
...
/****************************************************
函数功能：LCD上显示24*24的汉字和26*24的字符
参数说明： Row_dr  汉字和字符显示行首地址
           Col_dr  汉字和字符显示列首地址
           forecolor   前景颜色
           backcolor   背景颜色
           Str             
****************************************************/ 
void LCD_display_char24(u16 Row_addr,u16 Col_addr,u16 forecolor,u16 backcolor,char *Str,...)
{
  va_list arg_ptr;
  u8 c1,c2,temp;  
  s8 tmpBuf[64];      // LCD显示数据缓冲区
  u8 i=0,k,a,uLen;
  u16 uRow,uCol,j;
  ...
  uLen = (u8)vsprintf((char *)tmpBuf, (char *)Str, arg_ptr);
  va_end(arg_ptr); 
  ...
  while(i<uLen)
  {
   ...
   if(c1 <= 128) // ASCII
   {
    LCD_Set_Window(uRow,uCol,16,24);
    LCD_WriteRAM_Prepare();	//开始写入GRAM	
    for(a=0;a<48;a++) 
		{
			temp=ASC_MSK_24[(c1-0x20)*48 +a ];
			for(k=0;k<8;k++)
    	{
      	if(temp&0x80)
   ...
   {
    for(j=0;j<sizeof(GB_24)/sizeof(GB_24[0]);j++)	// 查找定位
		{
			if((c1 == GB_24[j].Index[0])&&(c2 == GB_24[j].Index[1]))
				break;
		}
    LCD_Set_Window(uRow,uCol,24,24);
    LCD_WriteRAM_Prepare();	//开始写入GRAM
    for(a=0;a<72;a++)
    {
     temp=GB_24[j].Msk[a];
